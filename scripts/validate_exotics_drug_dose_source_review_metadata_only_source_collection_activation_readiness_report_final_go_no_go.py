...
REQUIRED_FILES = [
    "backend/exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go.py",
    "docs/clinical_data/EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_REPORT_FINAL_GO_NO_GO_V1.md",
    "docs/clinical_data/EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_REPORT_FINAL_GO_NO_GO_MATRIX_V1.csv",
    "docs/clinical_data/EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_REPORT_FINAL_GO_NO_GO_CHECKLIST_V1.csv",
    "docs/clinical_data/EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_REPORT_FINAL_GO_NO_GO_V1.csv",
    "scripts/validate_exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go.py",
]
...
REQUIRED_PREVIOUS_VALIDATORS = [
    ...
    "scripts/validate_exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness.py",
...
def load_module():
    path = ROOT / "backend/exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go.py"
    spec = importlib.util.spec_from_file_location("activation_report_final_go_no_go", str(path))
...
def assert_text_markers() -> None:
    backend = read("backend/exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go.py")
    doc = read("docs/clinical_data/EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_REPORT_FINAL_GO_NO_GO_V1.md")
    ...
    required_backend = [
        "exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go_v1",
        "build_activation_readiness_report_final_go_no_go",
        GOVERNANCE_DECISION,
    ...
    required_doc = [
        "Exotics Drug Dose Source Review Metadata-only Source Collection Activation Readiness Report Final Go/No-Go V1",
        "metadata_only_source_collection_activation_readiness_report_final_go_no_go_schema_only_not_activation",
        GOVERNANCE_DECISION,
        "no source collection activation",
        "no drug dose",
        "decision=GO_TO_EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_GOVERNANCE_SIGNOFF_V1",
    ]
    for marker in required_doc:
        if marker not in doc:
            fail("doc missing marker: %s" % marker)
    if "validate_exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go.py" not in ci:
        fail("ci_static_checks missing final Go/No-Go validator")
    if "Exotics Drug Dose Source Review Metadata-only Source Collection Activation Readiness Report Final Go/No-Go V1" not in smoke:
        fail("smoke missing final Go/No-Go block")


def assert_matrix() -> None:
    path = ROOT / "docs/clinical_data/EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_REPORT_FINAL_GO_NO_GO_MATRIX_V1.csv"
    ...
    for row in rows:
        if row.get("governance_decision") != GOVERNANCE_DECISION:
            fail("matrix row must remain NO-GO to activation")
        if row.get("final_go_no_go_status") != GOVERNANCE_DECISION:
            fail("final_go_no_go_status must remain NO-GO to activation")
...
def assert_module_behavior() -> None:
    module = load_module()
    report = module.build_activation_readiness_report_final_go_no_go()
    if report.get("mode") != "exotics_drug_dose_source_review_metadata_only_source_collection_activation_readiness_report_final_go_no_go_v1":
        fail("mode mismatch")
    if report.get("current_level") != "metadata_only_source_collection_activation_readiness_report_final_go_no_go_schema_only_not_activation":
        fail("current_level mismatch")
    if report.get("governance_decision") != GOVERNANCE_DECISION:
    ...
    expected_true = [
        ...
        "activation_readiness_report_final_go_no_go_defined",
        "requires_human_review",
        "clinician_signoff_required",
        "not_client_facing",
    ]
...
def main() -> None:
    ...
    print("VALIDATOR=PASS Exotics Drug Dose Source Review Metadata-only Source Collection Activation Readiness Report Final Go/No-Go V1")
