...
def assert_no_forbidden_snippets(text: str) -> None:
    for snippet in FORBIDDEN_SNIPPETS:
        require(snippet not in text, "forbidden persistence design snippet found: {0}".format(snippet))


def main() -> int:
    print("Validating Treatment Framework Signed Review State Persistence Design V1")
    ...
    py_compile.compile(str(ROOT / "scripts/validate_treatment_framework_signed_review_state_persistence_design.py"), doraise=True)

    doc = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_DESIGN_V1.md")
    checklist = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_DESIGN_CHECKLIST_V1.csv")
    go_no_go = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_DESIGN_GO_NO_GO_V1.csv")
    ...
    previous_risk_doc = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_RISK_REVIEW_V1.md")
    ...
    require(
        ...
        "smoke must call persistence design after persistence risk review",
    )

    require("TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_MODE" in signed_backend, "signed review state backend mode missing")
    require('"writes_database": False' in signed_backend, "signed review state backend safety missing writes_database false")
    require('"persists_signed_review_state": False' in signed_backend, "signed review state backend safety missing persistence false")
    ...
    require("TreatmentFrameworkSignedReviewStatePanel" in ui, "signed review state UI panel missing")
    require("GO_TO_TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_DESIGN_V1" in previous_risk_doc, "previous risk review decision missing")
    ...
    print("PASS: Treatment Framework Signed Review State Persistence Design V1")
    print("persistence_design_only=true")
    ...
    print("signed_review_state_persistence_dry_run_required_first=true")
    ...
    print("decision=GO_TO_TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_DRY_RUN_V1")
    return 0
