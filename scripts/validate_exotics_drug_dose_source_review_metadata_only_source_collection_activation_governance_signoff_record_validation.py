...
NEXT = "GO_TO_EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_GOVERNANCE_SIGNOFF_RECORD_VALIDATION_REPORT_V1"
...
FILES = {
    "backend": ROOT / "backend" / "exotics_drug_dose_source_review_metadata_only_source_collection_activation_governance_signoff_record_validation.py",
    "doc": ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_GOVERNANCE_SIGNOFF_RECORD_VALIDATION_V1.md",
    "matrix": ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_GOVERNANCE_SIGNOFF_RECORD_VALIDATION_MATRIX_V1.csv",
    "checklist": ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_GOVERNANCE_SIGNOFF_RECORD_VALIDATION_CHECKLIST_V1.csv",
    "go_no_go": ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_GOVERNANCE_SIGNOFF_RECORD_VALIDATION_GO_NO_GO_V1.csv",
...
def load_backend_module():
    ...
    spec = importlib.util.spec_from_file_location("exotics_activation_governance_signoff_record_validation", str(path))
...
def assert_backend() -> None:
    ...
    required = [
        ...
        "build_validation_row", "build_matrix", "quality_gate",
    ...
    if gate.get("collection_execution_allowed_now") is not False:
        fail("collection execution must remain false")
...
def assert_docs_and_hooks() -> None:
    doc = read(FILES["doc"])
    required_doc = [
        "Exotics Drug Dose Source Review Metadata-only Source Collection Activation Governance Signoff Record Validation V1",
        GOVERNANCE_DECISION,
        "collection_activation_allowed_now=false",
        "collection_execution_started=false",
        "dose_output_enabled=false",
    ...
    if not load_rows(FILES["checklist"]):
        fail("checklist empty")
    if not load_rows(FILES["go_no_go"]):
        fail("go/no-go empty")
    ci = read(FILES["ci"])
    smoke = read(FILES["smoke"])
    validator_name = "validate_exotics_drug_dose_source_review_metadata_only_source_collection_activation_governance_signoff_record_validation.py"
...
def main() -> None:
    ...
    print("VALIDATOR=PASS Exotics Drug Dose Source Review Metadata-only Source Collection Activation Governance Signoff Record Validation V1")
