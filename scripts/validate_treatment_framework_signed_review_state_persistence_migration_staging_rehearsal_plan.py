...
FORBIDDEN_SNIPPETS = [
    ...
    "active_migration_file_created=true",
    "production_migration_apply_allowed=true",
...
def assert_no_forbidden_snippets(text: str) -> None:
    for snippet in FORBIDDEN_SNIPPETS:
        require(snippet not in text, "forbidden staging rehearsal plan snippet found: {0}".format(snippet))
...
def assert_no_active_0010_migration() -> None:
    ...
    require(not active, "active 0010 migration file is forbidden in staging rehearsal plan: {0}".format(", ".join(active)))


def main() -> int:
    print("Validating Treatment Framework Signed Review State Persistence Migration Staging Rehearsal Plan V1")
    assert_target_scope()
    ...
    py_compile.compile(str(ROOT / "scripts/validate_treatment_framework_signed_review_state_persistence_migration_staging_rehearsal_plan.py"), doraise=True)

    doc = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_STAGING_REHEARSAL_PLAN_V1.md")
    checklist = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_STAGING_REHEARSAL_PLAN_CHECKLIST_V1.csv")
    go_no_go = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_STAGING_REHEARSAL_PLAN_GO_NO_GO_V1.csv")
    ci = read("scripts/ci_static_checks.sh")
    smoke = read("scripts/smoke_petmed.sh")
    apply_readiness_doc = read("docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_APPLY_READINESS_REVIEW_V1.md")
    ...
    require_tokens("checklist", checklist, [
        "staging_rehearsal_plan_only",
        "staging_rehearsal_execution_allowed_false",
        "staging_rehearsal_dry_run_required_first",
    ...
    require_tokens("ci", ci, [
        ...
        "signed review state persistence migration staging rehearsal plan markers",
    ...
    for stage_scoped in [
        ...
        require(stage_scoped not in optional, "stage-scoped validator must not be re-run in this staging rehearsal plan CI: {0}".format(stage_scoped))
    ...
    require_tokens("smoke", smoke, [
        ...
        "GO_TO_TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_APPLY_READINESS_REVIEW_V1",
        "GO_TO_TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_STAGING_REHEARSAL_DRY_RUN_V1",
    ])
    require(
        re.search(
            r"\ncheck_treatment_framework_signed_review_state_persistence_migration_apply_readiness_review_v1\s*\ncheck_treatment_framework_signed_review_state_persistence_migration_staging_rehearsal_plan_v1\s*\n",
            smoke,
        ) is not None,
        "smoke must call staging rehearsal plan after apply readiness review",
    )

    require("GO_TO_TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_STAGING_REHEARSAL_PLAN_V1" in apply_readiness_doc, "previous apply readiness decision missing")
    require("revision = \"0010_treatment_framework_signed_review_states\"" in inactive_draft or "revision = \"0010_tfsrs\"" in inactive_draft, "inactive 0010 draft revision missing")
    ...
    print("PASS: Treatment Framework Signed Review State Persistence Migration Staging Rehearsal Plan V1")
    print("staging_rehearsal_plan_only=true")
    print("staging_rehearsal_execution_allowed=false")
    print("staging_rehearsal_dry_run_required_first=true")
    ...
    print("decision=GO_TO_TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_STAGING_REHEARSAL_DRY_RUN_V1")
    return 0
