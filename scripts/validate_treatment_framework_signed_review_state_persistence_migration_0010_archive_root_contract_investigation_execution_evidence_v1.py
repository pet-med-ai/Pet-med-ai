#!/usr/bin/env python3
"""Validate PMAI-P0-04 Archive Root Contract Investigation Execution Evidence V1."""
...
DOC = 'docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_0010_ARCHIVE_ROOT_CONTRACT_INVESTIGATION_EXECUTION_EVIDENCE_V1.md'
CHECKLIST = 'docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_0010_ARCHIVE_ROOT_CONTRACT_INVESTIGATION_EXECUTION_EVIDENCE_V1_CHECKLIST_V1.csv'
GO_NO_GO = 'docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_0010_ARCHIVE_ROOT_CONTRACT_INVESTIGATION_EXECUTION_EVIDENCE_V1_GO_NO_GO_V1.csv'
TEST_MATRIX = 'docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_0010_ARCHIVE_ROOT_CONTRACT_INVESTIGATION_EXECUTION_EVIDENCE_V1_TEST_MATRIX_V1.csv'
AUTHORIZATION_DOC = 'docs/clinical_data/TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_MIGRATION_0010_ARCHIVE_ROOT_CONTRACT_INVESTIGATION_AUTHORIZATION_REVIEW_V1.md'
...
VALIDATOR = 'scripts/validate_treatment_framework_signed_review_state_persistence_migration_0010_archive_root_contract_investigation_execution_evidence_v1.py'
...
EXPECTED_HEAD = '7e5e4008ed3613b566aebcf1fc0d12e527ac816c'
...
EXPECTED_FINAL_CI_SHA256 = 'd6cae61ff10138ae48be1832291aeefc19442ac68b323d4153939d1fbf19ea2d'
...
FALSE_MARKERS = (
    ...
    'current_archive_root_contract_investigation_execution_authorized',
    'current_archive_listing_attempt_authorized',
    'authorization_reuse_allowed',
    'investigator_reexecution_allowed',
    'investigator_v2_creation_authorized',
    'investigator_v2_execution_authorized',
    'corrected_restore_runner_design_authorized',
...
TRUE_MARKERS = (
    ...
    'route_b_fresh_restore_governance_decision_retained',
...
REQUIRED_TEST_IDS = {
    'PMAI-P0-04-ARCI-EXEC-EVID-T{:03d}'.format(number)
...
def main() -> int:
    ...
    required = {
        ...
        'sanitized_result_field_count': str(len(EXPECTED_RESULT)),
        'archive_member_count': '29',
        'archive_uncompressed_size_bytes': '196874',
        'regular_file_count': '26',
        'directory_entry_count': '3',
    ...
    need(len(json_blocks) == 1, 'sanitized JSON block count')
    result = json.loads(json_blocks[0])
    need(result == EXPECTED_RESULT, 'sanitized JSON exact result')
    canonical = json.dumps(result, ensure_ascii=True, sort_keys=True, separators=(',', ':'))
    need(canonical == json_blocks[0], 'sanitized JSON canonical encoding')
    need(hashlib.sha256(canonical.encode('utf-8')).hexdigest() == EXPECTED_RESULT_SHA256, 'sanitized JSON digest')
    need(len(result) == 33, 'sanitized JSON field count')
    ...
    need(len(checklist) == 60, 'checklist row count')
    ...
    need(len(decisions) == 20, 'Go/No-Go row count')
    ...
    need(by_gate['normalized path violations required for success']['current'] == '29', 'Go/No-Go violations')
    ...
    need(sha256_path(ROOT / IMPLEMENTATION) == EXPECTED_IMPLEMENTATION_SHA256, 'investigator hash changed')
    need(IMPLEMENTATION not in '\n'.join(python_lines(ci)), 'investigator executed by CI')
    ...
    marker_line = '# PMAI-P0-04 archive root contract investigation execution evidence v1'
    ...
    auth_pointer = {
        'subsequent_authorization_review_commit': EXPECTED_HEAD,
    ...
    print('PASS: PMAI-P0-04 Archive Root Contract Investigation Execution Evidence V1')
    print('stage_id=PMAI-P0-04')
    print('substage=ARCHIVE_ROOT_CONTRACT_INVESTIGATION_EXECUTION_EVIDENCE_V1')
    print('evidence_status=COMPLETE_SINGLE_METADATA_ATTEMPT_HOLD')
    ...
    print('archive_member_count=29')
    print('normalized_path_violation_count=29')
    ...
    print('next_action=SEPARATE_STRUCTURAL_PREDICATE_REVIEW_GOVERNANCE_DECISION_REQUIRED_BEFORE_ANY_NEW_INVESTIGATION')
    return 0
