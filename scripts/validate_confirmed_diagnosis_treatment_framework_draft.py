...
def main() -> int:
    ...
    require("LEGACY_SMOKE_COMPAT_RABBIT_GI_TREE_PATH_V1" in smoke_text, "missing legacy rabbit GI tree_path compatibility marker")
    require("LEGACY_SMOKE_COMPAT_LIZARD_UVB_TREE_PATH_V1" in smoke_text, "missing legacy lizard UVB tree_path compatibility marker")
    require("apply_legacy_smoke_current_backend_compatibility_patch" in smoke_text, "missing legacy smoke compatibility patch function")
    require("兔|胃肠" in smoke_text, "missing rabbit GI tree_path compatibility replacement")
    require("蜥蜴|UVB" in smoke_text, "missing lizard UVB tree_path compatibility replacement")
    require("legacy_repo_dir" in smoke_text, "legacy cumulative smoke must run in isolated baseline repo")
    require("git archive" in smoke_text, "legacy cumulative smoke missing baseline git archive checkout")
    require('cd "${legacy_repo_dir}"' in smoke_text, "legacy cumulative smoke must cd into isolated baseline repo")
