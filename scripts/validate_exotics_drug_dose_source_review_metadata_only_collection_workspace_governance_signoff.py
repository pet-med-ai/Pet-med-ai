...
HELPER = ROOT / "backend" / "exotics_drug_dose_source_review_metadata_only_collection_workspace_governance_signoff.py"
DOC = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_V1.md"
MATRIX = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_MATRIX_V1.csv"
CHECKLIST = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_CHECKLIST_V1.csv"
GO_NO_GO = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_GO_NO_GO_V1.csv"
VALIDATOR = ROOT / "scripts" / "validate_exotics_drug_dose_source_review_metadata_only_collection_workspace_governance_signoff.py"
...
def load_helper():
    ...
    spec = importlib.util.spec_from_file_location("exotics_workspace_governance_signoff", str(HELPER))
...
def assert_matrix(module) -> None:
    ...
    for row in rows:
        if row.get("metadata_only_policy") != "metadata_only_no_medication_values":
            fail("matrix metadata policy mismatch")
        if row.get("value_capture_blocker_status") != "enabled_forbidden_value_columns_absent":
            fail("value capture blocker not enabled")
        ...
        if row.get("governance_signoff_status") != "defined_not_signed":
            fail("governance signoff must be defined_not_signed")
...
def assert_helper_summary(module) -> None:
    summary = module.summarize_governance_signoff()
    checks = {
    ...
    if summary.get("source_review_status") != "metadata_only_workspace_governance_signoff_defined_no_collection_execution":
...
def assert_text_requirements() -> None:
    ...
    for needle in (
        ...
        "metadata_only_workspace_governance_signoff_schema_only_not_collection_execution",
        "dose_output_enabled=false",
        "collection_execution_started=false",
        "governance_signoff_completed=false",
        "requires_human_review=true",
    ...
    ci = read(CI)
    smoke = read(SMOKE)
    validator_name = "validate_exotics_drug_dose_source_review_metadata_only_collection_workspace_governance_signoff.py"
...
def main() -> None:
    ...
    print("VALIDATOR=PASS Exotics Drug Dose Source Review Metadata-only Collection Workspace Governance Signoff V1")
