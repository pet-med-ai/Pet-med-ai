...
BACKEND = ROOT / "backend" / "exotics_drug_dose_source_review_metadata_only_collection_workspace_governance_signoff_record_final_go_no_go.py"
DOC = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_RECORD_FINAL_GO_NO_GO_V1.md"
MATRIX = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_RECORD_FINAL_GO_NO_GO_MATRIX_V1.csv"
CHECKLIST = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_RECORD_FINAL_GO_NO_GO_CHECKLIST_V1.csv"
GO_NO_GO = ROOT / "docs" / "clinical_data" / "EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_COLLECTION_WORKSPACE_GOVERNANCE_SIGNOFF_RECORD_FINAL_GO_NO_GO_V1.csv"
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("final_go_no_go", str(BACKEND))
...
def assert_backend() -> None:
    module = load_module()
    summary = module.build_final_go_no_go_summary()
    if summary.get("mode") != "exotics_drug_dose_source_review_metadata_only_collection_workspace_governance_signoff_record_final_go_no_go_v1":
        fail("mode mismatch")
    if summary.get("governance_decision") != "NO_GO_TO_COLLECTION_EXECUTION":
        fail("governance decision must remain NO-GO")
    ...
    required_false = [
        "writes_database", "source_collection_execution", "collection_execution_started", "collection_execution_allowed_now",
...
def assert_docs_and_csv() -> None:
    ...
    for snippet in (
        ...
        "GO_TO_EXOTICS_DRUG_DOSE_SOURCE_REVIEW_METADATA_ONLY_SOURCE_COLLECTION_ACTIVATION_READINESS_V1",
    ):
    ...
    for row in rows:
        if row.get("governance_decision") != "NO_GO_TO_COLLECTION_EXECUTION":
            fail("all rows must remain NO_GO_TO_COLLECTION_EXECUTION")
...
def assert_hooks() -> None:
    ci = read(CI)
    smoke = read(SMOKE)
    validator_name = "validate_exotics_drug_dose_source_review_metadata_only_collection_workspace_governance_signoff_record_final_go_no_go.py"
...
def main() -> None:
    ...
    assert_docs_and_csv()
    assert_hooks()
    print("VALIDATOR=PASS Exotics Drug Dose Source Review Metadata-only Collection Workspace Governance Signoff Record Final Go/No-Go V1")
