...
def main() -> int:
    ...
    need(len(targets) == 147 and len(targets) == len(set(targets)), "CI TARGETS canonical")
    ...
    need(
        len(python_lines(ci)) == 30
        and python_lines(ci)[-10:-2] == [command, authorization_review_command, implementation_preparation_command, implementation_authorization_review_command, fresh_target_preparation_command, fresh_target_authorization_review_command, fresh_target_external_execution_authorization_command, fresh_target_execution_evidence_command],
        "CI command order",
    )
