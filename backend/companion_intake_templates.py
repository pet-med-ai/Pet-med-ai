...
INTAKE_DIR = ROOT / "knowledge-base" / "companion" / "intake"


class CompanionIntakeTemplateError(RuntimeError):
    pass
...
def _load_json(filename: str) -> Dict[str, Any]:
    ...
    if not path.exists():
        raise CompanionIntakeTemplateError(f"companion intake template not found: {path}")
    with path.open("r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, dict):
        raise CompanionIntakeTemplateError(f"{filename} must be a JSON object")
    return data


def _validate_string_list(template_key: str, field: str, value: Any, allow_empty: bool = False) -> None:
    if not isinstance(value, list):
        raise CompanionIntakeTemplateError(f"{template_key}.{field} must be a list")
    if not allow_empty and not value:
        raise CompanionIntakeTemplateError(f"{template_key}.{field} must not be empty")
    for idx, item in enumerate(value, 1):
        if not isinstance(item, str) or not item.strip():
            raise CompanionIntakeTemplateError(f"{template_key}.{field}[{idx}] must be a non-empty string")


def _validate_question(template_key: str, section_key: str, item: Any, idx: int) -> None:
    if not isinstance(item, dict):
        raise CompanionIntakeTemplateError(f"{template_key}.{section_key}.questions[{idx}] must be an object")
    for field in ("key", "label", "answer_type"):
        if not isinstance(item.get(field), str) or not item.get(field, "").strip():
            raise CompanionIntakeTemplateError(f"{template_key}.{section_key}.questions[{idx}].{field} must be a non-empty string")
    if "required" in item and not isinstance(item.get("required"), bool):
        raise CompanionIntakeTemplateError(f"{template_key}.{section_key}.questions[{idx}].required must be boolean")
    ...
    if "clinical_reason" in item and not isinstance(item.get("clinical_reason"), str):
        raise CompanionIntakeTemplateError(f"{template_key}.{section_key}.questions[{idx}].clinical_reason must be a string")


def _validate_template(template_key: str, data: Dict[str, Any]) -> None:
    for field in ("key", "label", "summary"):
        if not isinstance(data.get(field), str) or not data.get(field, "").strip():
            raise CompanionIntakeTemplateError(f"{template_key}.{field} must be a non-empty string")
    if data.get("key") != template_key:
        raise CompanionIntakeTemplateError(f"{template_key}.key mismatch: {data.get('key')!r}")
    ...
    if not isinstance(sections, list) or not sections:
        raise CompanionIntakeTemplateError(f"{template_key}.sections must be a non-empty list")
    for idx, section in enumerate(sections, 1):
        if not isinstance(section, dict):
            raise CompanionIntakeTemplateError(f"{template_key}.sections[{idx}] must be an object")
        for field in ("key", "title", "priority"):
            if not isinstance(section.get(field), str) or not section.get(field, "").strip():
                raise CompanionIntakeTemplateError(f"{template_key}.sections[{idx}].{field} must be a non-empty string")
        questions = section.get("questions")
        if not isinstance(questions, list) or not questions:
            raise CompanionIntakeTemplateError(f"{template_key}.sections[{idx}].questions must be a non-empty list")
...
def _validate_index(data: Dict[str, Any]) -> None:
    ...
    if not isinstance(templates, list) or not templates:
        raise CompanionIntakeTemplateError("companion/intake/index.json templates must be a non-empty list")
    for key in templates:
        if not isinstance(key, str) or not key.strip():
            raise CompanionIntakeTemplateError("companion/intake/index.json templates must be non-empty strings")
    for mapping_field in ("species_to_template", "group_to_template"):
        mapping = data.get(mapping_field)
        if not isinstance(mapping, dict):
            raise CompanionIntakeTemplateError(f"companion/intake/index.json {mapping_field} must be an object")
        for source, target in mapping.items():
            if not isinstance(source, str) or not isinstance(target, str):
                raise CompanionIntakeTemplateError(f"{mapping_field} key/value must be strings")
            if target not in templates:
                raise CompanionIntakeTemplateError(f"{mapping_field}.{source} points to unknown template: {target}")
...
@lru_cache(maxsize=1)
def load_companion_intake_templates() -> Dict[str, Dict[str, Any]]:
...
def reload_companion_intake_templates() -> Dict[str, Dict[str, Any]]:
    load_intake_index.cache_clear()
    load_companion_intake_templates.cache_clear()
    return load_companion_intake_templates()


def intake_key_for_features(features: Dict[str, Any]) -> str:
    index = load_intake_index()
    species_to_template = index.get("species_to_template") or {}
    group_to_template = index.get("group_to_template") or {}

    species = str(features.get("species") or "").strip()
    if species in species_to_template:
        return str(species_to_template[species])

    group = str(features.get("species_group") or "").strip()
    return str(group_to_template.get(group) or "")
...
def build_companion_structured_intake(features: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(features, dict):
        return None
    if features.get("is_exotic"):
        return None
    ...
    template = load_companion_intake_templates().get(template_key)
    if not template:
        return None
