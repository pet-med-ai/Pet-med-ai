...
# --- Treatment Framework Clinician Review Workflow V1 endpoint: start ---
@router.post("/dry-run/confirmed-diagnosis/treatment-framework/review", response_model=dict)
def build_treatment_framework_clinician_review_workflow_dry_run(
    data: Dict[str, Any],
    db: Session = Depends(get_db),
    user=Depends(get_current_user),
):
    try:
        from backend.treatment_framework_clinician_review_workflow import (
            TREATMENT_FRAMEWORK_CLINICIAN_REVIEW_WORKFLOW_MODE,
            build_treatment_framework_clinician_review_workflow,
            treatment_framework_clinician_review_workflow_safety_flags,
        )
    except ModuleNotFoundError:
        from treatment_framework_clinician_review_workflow import (
            TREATMENT_FRAMEWORK_CLINICIAN_REVIEW_WORKFLOW_MODE,
            build_treatment_framework_clinician_review_workflow,
            treatment_framework_clinician_review_workflow_safety_flags,
        )
    ...
    try:
        workflow = build_treatment_framework_clinician_review_workflow(
            payload,
            case_context=case_payload,
        )
    except ValueError as exc:
    ...
    api_safety = _safety_flags(dry_run=True)
    workflow_safety = treatment_framework_clinician_review_workflow_safety_flags()
    combined_safety = {**api_safety, **workflow_safety}
    return {
...
# --- Treatment Framework Clinician Review Workflow V1 endpoint: end ---
# --- Treatment Framework Signed Review State Dry Run V1 endpoint: start ---
@router.post("/dry-run/confirmed-diagnosis/treatment-framework/signed-review-state/build", response_model=dict)
def build_treatment_framework_signed_review_state_dry_run(
    data: Dict[str, Any],
    db: Session = Depends(get_db),
    user=Depends(get_current_user),
):
    try:
        from backend.treatment_framework_signed_review_state import (
        ...
        from treatment_framework_signed_review_state import (
            TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_MODE,
            build_treatment_framework_signed_review_state,
            treatment_framework_signed_review_state_safety_flags,
        )
    ...
    try:
        signed_state = build_treatment_framework_signed_review_state(
            payload,
            case_context=case_payload,
        )
    except ValueError as exc:
    ...
    api_safety = _safety_flags(dry_run=True)
    signed_state_safety = treatment_framework_signed_review_state_safety_flags()
    combined_safety = {**api_safety, **signed_state_safety}
    return {
...
@router.post("/dry-run/confirmed-diagnosis/treatment-framework/signed-review-state/persistence/prepare", response_model=dict)
def build_treatment_framework_signed_review_state_persistence_dry_run(
    ...
    try:
        from backend.treatment_framework_signed_review_state_persistence import (
        ...
        from treatment_framework_signed_review_state_persistence import (
            TREATMENT_FRAMEWORK_SIGNED_REVIEW_STATE_PERSISTENCE_DRY_RUN_MODE,
            build_treatment_framework_signed_review_state_persistence_dry_run,
            treatment_framework_signed_review_state_persistence_dry_run_safety_flags,
        )
    ...
    try:
        persistence_preview = build_treatment_framework_signed_review_state_persistence_dry_run(
            payload,
            case_context=case_payload,
        )
    except ValueError as exc:
    ...
    api_safety = _safety_flags(dry_run=True)
    persistence_safety = treatment_framework_signed_review_state_persistence_dry_run_safety_flags()
    combined_safety = {**api_safety, **persistence_safety}
    return {
