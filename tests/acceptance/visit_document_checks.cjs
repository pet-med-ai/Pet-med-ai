...
async function main(){
 ...
 for(const [label,name] of [['导出入院/住院记录 DOCX','admission'],['导出出院小结 DOCX','discharge']]){
 ...
 assert.equal(mutations.length,beforeWrites);await record('both_docx_downloads_contain_corrected_case_without_write');
